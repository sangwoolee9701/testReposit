<template>
  ...
  <!-- 보라색 이름은 내가 작명한거 흰색은 밑에 데이터 -->
  <!-- App.vue에서 쓰는 데이터도 props보내려면 :데이바인딩르로 작명="데이터로" 보내고 만든 컴포넌트().vue에서 쓰면 됨-->
  <Modal @close="모달창열렸니 = false" :원룸v="원룸들" :누른거="누른거" :모달창열렸니="모달창열렸니"/>

  <!-- 부모가 메세지 받을 땐 @작명한거="" -->
  <!-- 누른거 뒤에 $event는 openModal 작명 뒤에 ,쓴 데이터들을 $event로 써서 나타낼수있음-->
  <Card @openModal="모달창열렸니1 = true; 누른거1 = $event" :원룸="원룸들[i]" v-for="(type,i) in 원룸들" :key="type" />
  <!-- <Card :원룸="원룸들[0]" />
  <Card :원룸="원룸들[1]" />
  <Card :원룸="원룸들[2]" />
  <Card :원룸="원룸들[3]" />
