<template>
  ...
  <Card @openModal="모달창열렸니 = true; 누른거 = $event" :원룸="원룸들[i]" v-for="(typex,i) in 원룸들" :key="typex" />
  <!-- <Card :원룸="원룸들[0]" />
  <Card :원룸="원룸들[1]" />
  <Card :원룸="원룸들[2]" />
  <Card :원룸="원룸들[3]" />
...
// 변수로 만들어서 하든가 그냥 하든가(aa는 내가 작명)
// import roomdata from './assets/oneroom.js';
import aa from './assets/oneroom.js';
...
export default {
  ...
  data(){
    return{
      ...
      // 원룸들 : roomdata,
      원룸들: aa,
      원룸들오리지널 : [...aa], /* array/object 데이터의 별개의 사본을 만들려면 [...array자료]이방식으로 만들어함 */
  ...
  // 라이프사이클 훅 함수 (mounted이런건 정해져있는 함수임 mounted 즉 랜더링 되고 함수 실행해라 뜻임)
  mounted(){
    setTimeout(()=>{
